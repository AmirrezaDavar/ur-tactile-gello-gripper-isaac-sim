...
mypath = os.environ['HOME'] + '/training_data/lerobot/my_pusht_2images/data/chunk-000'
...
for file in onlyfiles:
    ...
    #cam_high
    observation_image_path = os.environ['HOME'] + '/training_data/lerobot/my_pusht_2images/videos/chunk-000/observation.images.top/'
    ...
    episode_dic['stats']['observation.images.top'] = video_stats
    shutil.rmtree(temp_img_path)

    #cam_low
    # Should be changed in 2 places!! line 144 and 172
    observation_image_path = os.environ['HOME'] + '/training_data/lerobot/my_pusht_2images/videos/chunk-000/observation.images.wrist/'
    video_path = observation_image_path + file.replace('parquet', '') + 'mp4'
    temp_img_path = observation_image_path + 'temp_imgs/'
    img_index = 0
    #video_data = [video_path + file.replace('parquet', '') + 'mp4']
    ...
    video_stats = {k: v if k == "count" else np.squeeze(v / 255.0, axis=0) for k, v in temp_video_stats.items()}
    video_stats = {k: v.tolist() for k, v in video_stats.items()}
    #episode_dic['stats']['observation.images.cam_low'] = video_stats
    episode_dic['stats']['observation.images.wrist'] = video_stats
    shutil.rmtree(temp_img_path)
